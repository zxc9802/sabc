...
import { researchSnapshotSchema } from "@/lib/storage/storage-schemas";
...
export async function POST(request: NextRequest, { params }: Context) {
  try {
    ...
    const parsed = researchSnapshotSchema.safeParse(await request.json());
    const { projectId } = await params;
    if (!parsed.success || parsed.data.projectId !== projectId) return badRequest();
    return await context.repository.saveResearchSnapshot(context.ownerId, parsed.data)
