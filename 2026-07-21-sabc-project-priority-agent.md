...
**Architecture:** Extend the existing sabc-rating-agent Next.js App Router scaffold. Keep the API key and Gemini calls in server-only modules while IndexedDB stores all business data in the browser. The model produces schema-validated facts, evidence states, dimension score proposals, veto risks, and question candidates; a pure TypeScript engine applies evidence caps, fixed weights, confidence rules, grade thresholds, and vetoes.
...
**Reference specification:** docs/superpowers/specs/2026-07-21-sabc-project-priority-agent-design.md

**Existing application:** D:\SABC评级智能体\sabc-rating-agent already contains a clean create-next-app scaffold and passes npm.cmd run build. Run all commands in Tasks 1 through 14 from that directory. All application file paths below are relative to sabc-rating-agent; the specification and this plan remain in the parent repository.
...
- lib/ai/yunwu-client.ts: server-only Gemini transport and grounding extraction
...
- scripts/smoke-yunwu.mjs: secret-safe live provider check
...
### Task 1: Adopt the existing Next.js 16 scaffold and add quality gates
...
- [ ] **Step 1: Verify the generated scaffold before changing it**
...
Expected: Next.js 16.2.10 compiles successfully and lists the existing static root route.
...
~~~json
{
  "scripts": {
    ...
    "test:watch": "vitest",
    "e2e": "playwright test",
    "smoke:ai": "node scripts/smoke-yunwu.mjs",
...
~~~dotenv
YUNWU_GEMINI_ENDPOINT=https://yunwu.ai/v1beta/models/gemini-3.1-flash-lite:generateContent
YUNWU_API_KEY=replace-with-a-newly-rotated-key
~~~
...
Update layout.tsx metadata to title “SABC 项目优先级评估” and description “面向小团队的可追溯项目投入建议”, keep it a Server Component, and set html lang="zh-CN". Replace the generated logo and template links in page.tsx with the exact heading and introduction asserted above. Keep @import "tailwindcss" at the top of globals.css, then define CSS variables for background, panel, text, muted text, borders, S/A/B/C colors, spacing, focus rings, and an 1180px centered layout.
...
export interface EvidenceItem {
  ...
  statement: string;
  state: EvidenceState;
  origin: "user_input" | "grounding" | "model_inference";
...
- the rule that URLs may appear only when supplied by grounding metadata
...
### Task 6: Add the Yunwu Gemini client and grounding parser

**Files:**
- Create: lib/ai/yunwu-client.ts
- Create: scripts/smoke-yunwu.mjs
- Test: lib/ai/yunwu-client.test.ts

- [ ] **Step 1: Write transport tests with a fake fetch**

Assert all of the following:

- POST goes to YUNWU_GEMINI_ENDPOINT.
- Authorization is Bearer plus YUNWU_API_KEY and neither value enters the request body.
- request contents contain system and user data as separate parts.
- useSearch adds tools containing google_search.
- JSON text is extracted from candidates[0].content.parts.
- source title and URL are extracted only from groundingChunks web entries.
- a non-2xx error throws ProviderError with status and a sanitized message that excludes the key and authorization header.
- missing endpoint or key fails before fetch.

- [ ] **Step 2: Run and verify failure**

~~~powershell
npm.cmd test -- lib/ai/yunwu-client.test.ts
~~~

Expected: FAIL because YunwuClient is missing.

- [ ] **Step 3: Implement the server-only client**

Start yunwu-client.ts with:
...
Create YunwuClient with injected fetch for tests and a 45-second AbortController timeout. Use Authorization: Bearer. Build the Gemini-native body with contents, generationConfig responseMimeType application/json, temperature 0.2, and tools google_search only when requested. Parse all text parts, deduplicate grounding URLs, and never accept URLs copied from model text as sources.

ProviderError exposes status and a safe Chinese message. It never includes the response authorization header, request body, API key, or full project text.

- [ ] **Step 4: Add the live smoke script**

The script reads YUNWU_GEMINI_ENDPOINT and YUNWU_API_KEY, sends the harmless prompt “仅返回 JSON：{\"ok\":true}”, prints only HTTP status, whether valid JSON text was returned, and whether grounding metadata was present. It exits nonzero on authentication, protocol, or parsing failure and never prints the key.
...
- [ ] **Step 6: Run the live smoke only with a newly rotated key**

In the current PowerShell session, set the new key without writing it to shell scripts:

~~~powershell
$env:YUNWU_GEMINI_ENDPOINT = "https://yunwu.ai/v1beta/models/gemini-3.1-flash-lite:generateContent"
$env:YUNWU_API_KEY = Read-Host "输入新密钥"
npm.cmd run smoke:ai
Remove-Item Env:YUNWU_API_KEY
~~~

Expected: HTTP 200 and valid JSON. Record whether grounding is supported. If the endpoint rejects Bearer authentication, stop this task and reconcile the provider's authenticated documentation before changing the client; do not guess by emitting the key in query strings.

- [ ] **Step 7: Commit**

~~~powershell
git add lib/ai/yunwu-client.ts lib/ai/yunwu-client.test.ts scripts/smoke-yunwu.mjs
git commit -m "feat: integrate Yunwu Gemini analysis"
~~~
...
Also assert that when research.needed is true, a grounded second request is made; if it returns no grounding source, evidence stays specific_unverified and response.researchStatus is "unavailable".
...
Return project identity, parsed analysis, scored assessment, selected next question, deterministic diff, grounding sources, and researchStatus as not_needed, grounded, or unavailable. Before scoring, remove any source URL that is not present in grounding metadata and downgrade its evidence state to specific_unverified. Accept a confirmed veto only when its evidence references an explicit user message or a verified grounding source; otherwise convert it to suspected. Reject project descriptions over 20,000 characters and conversation histories over 30 messages in the first release.
...
Use a desktop grid of 240px / minmax(420px, 1fr) / 340px. On screens below 900px, show project history in a collapsible top section, conversation next, and assessment last. Use text plus color for every grade. Use progress bars with aria-valuenow for dimensions and confidence. Preserve visible focus states and meet 4.5:1 text contrast. Do not add decorative animation that delays results.
...
The script requires a successful .next build, walks only .next/static and browser chunks under .next/server/app, reads YUNWU_API_KEY without printing it, and fails when the exact key or an Authorization Bearer prefix plus that key appears. If the environment key is absent, it scans for the test marker and obvious committed assignments matching YUNWU_API_KEY followed by a non-placeholder value.
